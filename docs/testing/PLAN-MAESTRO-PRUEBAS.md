...
- **Cubierto con pruebas automatizadas reales**: Auth, Challenges (parcial), Workout Log (parcial), Users/Perfil (parcial), **Posts/Feed (B2, completo)**, **Followers (completo)**, **Badges (completo)**, **Challenge Invites (completo)**, **Privacidad de posts en challenges privados / F8 (completo)**, **Chats / Mensajería directa 1:1 (completo a nivel unitario, ver limitaciones)**, **Reacciones y Comentarios sobre publicaciones / Bloque 3 (completo a nivel unitario, ver limitaciones)**.
...
### Casos agregados — Bloque 3: Reacciones y Comentarios (CP-53 en adelante)

Justificación: nuevo alcance sobre el módulo existente `src/workout-posts/`, sin CP base previo (CP-18/CP-19 solo cubren el contrato de Feed, no reacciones/comentarios). El init-schema ya traía `workout_post_likes` completo (sección "29. workout_post_likes") pero, igual que pasó con `spaces`/`space_members` antes de CP-44, sin ningún módulo leyéndolo ni escribiéndolo hasta ahora — el modelo de datos ("un tipo de reacción, una por usuario por post, PK compuesta") ya estaba decidido implícitamente por esa tabla, no se inventó uno nuevo. Comentarios no tenían tabla; se creó `workout_post_comments` (`2026-09-08-01-create-workout-post-comments.sql`) con la misma convención de soft-delete vía `is_active` que `space_messages`/`user_follows`/`workout_posts`.
...
| ID | Funcionalidad | Tipo | Prueba | Condiciones/Entrada | Resultado esperado | Prioridad | Cobertura automatizada | Estado |
|---|---|---|---|---|---|---|---|---|
| CP-53 | Reacciones | Funcional | Reaccionar a una publicación | Usuario autenticado, post público/propio existente, sin reacción previa | Se crea la fila en `workout_post_likes`; post inexistente → `404`; post `private` ajeno → `403`; el propio dueño sí puede reaccionar a su post privado | Alta | `workout-post-reactions.service.spec.ts` → describe `react` (7 tests) | Ejecutado — Aprobado |
| CP-54 | Reacciones | Funcional | Reacción duplicada | Usuario que ya reaccionó intenta reaccionar de nuevo | `409 ConflictException`, sin insertar una segunda fila; una condición de carrera (violación `23505` de la PK compuesta) se traduce al mismo `409`, mismo patrón que `FollowsService.follow` | Alta | mismo describe `react` (`"should throw ConflictException..."`, `"should translate a race-condition unique-violation..."`) | Ejecutado — Aprobado |
| CP-55 | Reacciones | Funcional | Quitar la reacción propia | Usuario con/sin reacción previa | Con reacción: se elimina solo la fila del usuario autenticado; sin reacción previa: `404 NotFoundException` | Alta | `workout-post-reactions.service.spec.ts` → describe `unreact` (2 tests) | Ejecutado — Aprobado |
| CP-56 | Reacciones | Funcional | Estado de reacciones de un post | `GET /workout-posts/:postId/reactions` | Devuelve `{ count, reactedByMe }` correctos para el usuario autenticado; post inexistente → `404` | Media | `workout-post-reactions.service.spec.ts` → describe `getSummary` (3 tests) | Ejecutado — Aprobado |
| CP-57 | Reacciones | Unitaria | Conteo por lote (Feed) | Varios `postId` a la vez | Una sola query agrupada (`GROUP BY workout_post_id`), no una `COUNT` por post; lista vacía no dispara ninguna query | Media | `workout-post-reactions.service.spec.ts` → describe `getCountsForPosts` (2 tests) | Ejecutado — Aprobado |
| CP-58 | Comentarios | Funcional | Crear un comentario | Usuario autenticado, post público/propio existente, contenido válido (1–500 caracteres) | Se persiste el comentario y se devuelve con el autor poblado (`username`/`displayName`/`profileImageUrl`, nunca email/password); post inexistente → `404`; post `private` ajeno → `403`; sin llamada a ningún servicio de moderación (ver decisión arriba); contenido vacío o >500 caracteres rechazado por `class-validator` antes de llegar al service (`ValidationPipe` global, `whitelist`/`forbidNonWhitelisted`) | Alta | `workout-post-comments.service.spec.ts` → describe `create` (3 tests) | Ejecutado — Aprobado |
| CP-59 | Comentarios | Funcional | Listar comentarios de un post | Post con comentarios activos, paginación por `after`/`limit` | Orden ascendente por `id` (más antiguos primero); `nextAfter` presente solo cuando hay más filas que el límite; post inexistente → `404`; solo comentarios `is_active = true` | Alta | `workout-post-comments.service.spec.ts` → describe `list` (3 tests) | Ejecutado — Aprobado |
| CP-60 | Comentarios | Funcional | Eliminar un comentario propio | Comentario propio vs. ajeno vs. inexistente | Propio: soft-delete (`is_active = false`); ajeno: `403 ForbiddenException` (`assertOwnership`, mismo helper de `src/auth/utils/`), sin tocar la fila; inexistente (o de otro post): `404` | Alta | `workout-post-comments.service.spec.ts` → describe `remove` (3 tests) | Ejecutado — Aprobado |
| CP-61 | Comentarios | Unitaria | Conteo por lote (Feed) | Varios `postId` a la vez | Una sola query agrupada, solo comentarios activos (`AND c.is_active = true`); lista vacía no dispara ninguna query | Media | `workout-post-comments.service.spec.ts` → describe `getCountsForPosts` (2 tests) | Ejecutado — Aprobado |
| CP-62 | Feed | Funcional | Extensión del contrato con reacciones/comentarios | `GET /feed` con posts existentes, con y sin `viewerId` | `likes_count`/`comments_count`/`liked_by_me` ahora se pueblan realmente (antes siempre `undefined`, ver CP-19) desde las mismas consultas por lote de CP-57/CP-61, más `getReactedPostIds` cuando hay `viewerId`; sin `viewerId` (caso defensivo, no ocurre en producción — el guard global exige auth) `liked_by_me` queda `false` sin llamar a `getReactedPostIds` | Alta | `workout-posts.service.spec.ts` → *"should populate likes_count/comments_count/liked_by_me..."*, *"should skip the per-viewer reacted lookup..."*; `feed.controller.spec.ts` → *"should pass the JWT-derived viewer id through as viewerId"* | Ejecutado — Aprobado |
| CP-63 | Reacciones/Comentarios | Funcional | Delegación de los controllers | Cada acción de `WorkoutPostReactionsController`/`WorkoutPostCommentsController` | Cada acción usa `user.sub` (JWT) como actor — nunca un campo del body/param — mismo patrón verificado en `follows.controller.spec.ts`/`spaces.controller.spec.ts` | Media | `workout-post-reactions.controller.spec.ts` (3/3) + `workout-post-comments.controller.spec.ts` (3/3) | Ejecutado — Aprobado |

**Resultados de ejecución:**

| Casos | Archivo / comando | Resultado | Estado | Evidencia |
|---|---|---|---|---|
| CP-53 – CP-57 | `workout-post-reactions.service.spec.ts` | `npx jest src/workout-posts/workout-post-reactions.service.spec.ts` | 14/14 tests pasan | Aprobado | Cubre react/unreact/getSummary/getCountsForPosts |
| CP-58 – CP-61 | `workout-post-comments.service.spec.ts` | `npx jest src/workout-posts/workout-post-comments.service.spec.ts` | 11/11 tests pasan | Aprobado | Cubre create/list/remove/getCountsForPosts |
| CP-62 | `workout-posts.service.spec.ts` + `feed.controller.spec.ts` | `npx jest src/workout-posts/workout-posts.service.spec.ts src/workout-posts/feed.controller.spec.ts` | 37/37 + 8/8 tests pasan | Aprobado | Incluye las 2 pruebas nuevas de `getFeed` y la de `viewerId` en el controller; el resto son las pruebas preexistentes (CP-18/19/20-31), todas siguen pasando sin modificación de comportamiento |
| CP-63 | `workout-post-reactions.controller.spec.ts` + `workout-post-comments.controller.spec.ts` | `npx jest src/workout-posts/workout-post-reactions.controller.spec.ts src/workout-posts/workout-post-comments.controller.spec.ts` | 3/3 + 3/3 tests pasan | Aprobado | Verifica delegación con `user.sub` en cada acción |
| Lint dirigido | `eslint` sobre los archivos nuevos/modificados de `workout-posts` | `npx eslint src/workout-posts/workout-post-reactions.* src/workout-posts/workout-post-comments.* src/workout-posts/workout-post-visibility.util.ts src/workout-posts/workout-posts.service.ts src/workout-posts/workout-posts.service.spec.ts src/workout-posts/feed.controller.ts src/workout-posts/feed.controller.spec.ts src/main.ts` | Sin errores nuevos (1 error preexistente en `workout-posts.service.spec.ts:469`, no tocado por este cierre, ya presente antes de Bloque 3) | Aprobado | — |
| Build completo | `nest build` | `npm run build` | Sin errores | Aprobado | — |
| Suite completa (con Bloque 3) | Todos los tests del backend | `npm run test` | **33 suites, 476 tests — todos pasan** (387 previos + 89 nuevos/modificados de Bloque 3) | Aprobado | Ver salida completa del comando |

**No ejecutado en esta sesión** (mismo motivo que Chats/Spaces — sin `.env`/credenciales de la base de datos de Azure en este sandbox):
- Aplicar `2026-09-08-01-create-workout-post-comments.sql` con `npm run db:migrate` contra la base real. `workout_post_likes` no necesita migración — ya existe en `database/init/2026-07-07-00-init-schema.sql` y, si la base real ya fue baseline-ada (ver nota de bootstrap en el README de `database/`), simplemente empieza a usarse.
